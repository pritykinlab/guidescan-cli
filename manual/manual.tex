...
\hl{(Sabatini ref)}.  The files for this example can be found in the
...
Here we describe a complete program to construct genome-wide
off-target databases for organisms. To make this example run quickly,
we will construct a databse for the C. elegans genome. An in
particular, we will use the RefSeq version provided by the National
Center for Biotechnology Information at the address:
